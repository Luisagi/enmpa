...
The raster layers for projections were obtained from [WorldClim](https://worldclim.org/):
...
can determine whether presences and absences can be separated better than randomly 
considering distinct environmental factors.
...
The function includes the flag `mode` to determine what strategy to use to combine 
predictors based on the responses defined in \code{type}. The options of mode are:
...
projection of these models. In this case we are projecting the models to the whole
area of interest.
...
the mean, median, and a weighted average (using Akaike weights) [@akaike1973; @wagenmakers2004].
...
An important step in understanding the ecological niches with these models is 
to explore variable response curves. The following lines of code help to do so:
...
Using a function from `enmpa` you can also explore variable importance in terms of
contribution.
...
(i.e., data that was not used during model calibration or for final model fitting).
